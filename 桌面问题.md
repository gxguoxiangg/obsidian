...
2.  如果可以打开任务管理器，停止 *AppReadiness* 服务，注销并重新登录进入
...
##	Q62 Microsoft Store 安装的国外APP Whatsapp 无法显示登录二维码

在Windows10有AppContainer机制，而UWP是在AppContainer中隔离运行。正是因为这个特性，UWP应用不可以访问本地回环地址（127.0.0.1）。这意味着，一些利用本地回环进行代理的工具，如小飞机无法对UWP应用代理网络访问。
...
##	Q63	Flash 提示当前版本已不再支持，请升级至最新的Flash大厅版

起因是，adobe flash 已退出国内，并将后续版本提交给国内公司运营，国内特供版有垃圾广告。而出现这个提示的原因是adobe检测到国内ip并且未安装特供版，解决方法如下：
...
##	Q64	删除word中表格后面多余的空白页

有时候，表格内容、格式固定，但是后面会多出一页空白页，无法正常删除。这是因为 word 中包含不可删除的结束段落，该段落有时会排到文档末尾的新空白页中，删除该页面的方法是将结束段落调整到前一页：
...
##	Q65	word中的表格，插入图片设置为嵌入式时，图片显示不全
...
1. 百兆网络，实际只使用了 1、2、3、6 这四芯。
   - 1、2芯：用于发送数据
   - 3、6芯：用于接收数据
2. 千兆网络中，7、8芯为一对，如果有问题，系统会自动协商降速到百兆。

即使对于百兆网络，**7、8芯有问题也可能会影响POE供电**。因为许多POE标准（如802.3af/at）会使用“空闲对”（4、5和7、8）来传输电力。如果7、8芯断路，可能导致设备无法受电或供电不稳定。
